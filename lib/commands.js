...
commands.waitFor = function(){
  ...
  // default
  opts.timeout = opts.timeout || config.getDOMConfig().timeout;
...
commands.elementByImageFile = function() {
  ...
  var cb = fargs.callback,
    imgPath = fargs.all[0],
    matchThreshold = fargs.all[1];
  fs.readFile(imgPath, function(err, fileData) {
    if (err) return cb(err);
    _this.elementByImage(fileData.toString('base64'), matchThreshold, cb);
  });
}

commands.elementByImage = function() {
  var fargs = utils.varargs(arguments);
  var _this = this;
  var cb = fargs.callback,
    base64Data = fargs.all[0],
    matchThreshold = fargs.all[1];
  if (typeof matchThreshold === 'undefined') {
    matchThreshold = 0.2;
  }
  this.takeScreenshot(function (err, base64Screenshot) {
    var data = {
      mode: 'matchTemplate'
      , firstImage: base64Screenshot
    ...
    _this._jsonWireCall({
      method: 'POST'
      , relPath: '/appium/compare_images'
      , data: data
      , cb: imageElementCallback(cb, _this)
    });
  });
}
